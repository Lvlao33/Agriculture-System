...
SET @sql = IF(@fk_name IS NOT NULL, 
    CONCAT('ALTER TABLE `comment` DROP FOREIGN KEY `', @fk_name, '`'),
...
SET @sql2 = IF(@has_knowledge_id > 0,
    'ALTER TABLE `comment` ADD CONSTRAINT `fk_comment_knowledge` FOREIGN KEY (`knowledge_id`) REFERENCES `knowledge` (`knowledge_id`) ON DELETE CASCADE',
    'SELECT "comment table does not have knowledge_id column" AS message');
