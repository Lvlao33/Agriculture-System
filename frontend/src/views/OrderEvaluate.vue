...
<script>
import { getOrderDetail, completeReview } from '../api/trade'
    ...
    async submitEvaluate() {
      ...
      try {
        ...
        if (error && typeof error === 'object') {
          if (error.flag === false) {
            const errorMsg = error.message || '提交评价失败'
            ...
            this.$message.error(error.message || '提交评价失败，请检查网络连接')
          }
        } else {
