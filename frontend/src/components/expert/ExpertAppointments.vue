...
export default {
  ...
  methods: {
    refresh() {
      this.$message.info('刷新预约接口：GET /api/expert/appointments')
    },
    accept(row) { 
      this.$message.success('已接受，接口：POST /api/expert/appointments/{id}/accept') 
    },
    reject(row) { 
      this.$message.error('已拒绝，接口：POST /api/expert/appointments/{id}/reject') 
    }
  }
}
</script>
